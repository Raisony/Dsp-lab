...
while stop == False:
    ...
    for n in range(BLOCKSIZE):
        # if fk == f[0]:
            y0[n] = b0 * x0[n] - a1 * y0[n-1] - a2 * y0[n-2] 
        # if fk == f[1]:
            y1[n] = b0 * x1[n] - a1 * y1[n-1] - a2 * y1[n-2]
        # if fk == f[2]:
            y2[n] = b0 * x2[n] - a1 * y2[n-1] - a2 * y2[n-2]
        # if fk == f[3]:
            y3[n] = b0 * x3[n] - a1 * y3[n-1] - a2 * y3[n-2]
        # if fk == f[4]:
            y4[n] = b0 * x4[n] - a1 * y4[n-1] - a2 * y4[n-2]
        # if fk == f[5]:
            y5[n] = b0 * x5[n] - a1 * y5[n-1] - a2 * y5[n-2]
        # if fk == f[6]:
            y6[n] = b0 * x6[n] - a1 * y6[n-1] - a2 * y6[n-2]
        # if fk == f[7]:
            y7[n] = b0 * x7[n] - a1 * y7[n-1] - a2 * y7[n-2]
        # if fk == f[8]:
            y8[n] = b0 * x8[n] - a1 * y8[n-1] - a2 * y8[n-2]
        # if fk == f[9]:
            y9[n] = b0 * x9[n] - a1 * y9[n-1] - a2 * y9[n-2]
        # if fk == f[10]:
            y10[n] = b0 * x10[n] - a1 * y10[n-1] - a2 * y10[n-2]
        # if fk == f[11]:
            y11[n] = b0 * x11[n] - a1 * y11[n-1] - a2 * y11[n-2] 
        # if fk == f[12]:
            y12[n] = b0 * x12[n] - a1 * y12[n-1] - a2 * y12[n-2]
