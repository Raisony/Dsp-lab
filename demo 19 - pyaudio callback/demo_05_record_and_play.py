...
def callback_record(binary_input_data, block_size, time_info, status):
    data = struct.unpack('h', binary_input_data)
...
def callback_play(binary_input_data, block_size, time_info, status):
    binary_data = wf.readframes(block_size)
    return(binary_data, pyaudio.paContinue)
